...
def play_round(cpu_agent, test_agents, win_counts, starting_moves):
    ...
    for first_move, second_move in starting_moves:

        games = sum([[Board(cpu_agent.player, agent.player),
                      Board(agent.player, cpu_agent.player)]
                    for agent in test_agents], [])

        # randomly chosen but consistently applied first moves
        for game in games:
        ...
        for game in games:
            ...
            #            replay.move_count,
            #            hmove,
            #            replay.get_player_location(replay.active_player),
            #            replay.get_player_location(replay.inactive_player),
            #            replay.get_blank_spaces(),
            #            replay.active_player == winner,
            #            file=f
            #            )
            #    replay.apply_move(hmove)
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    legal_moves = Board(None,None).get_legal_moves()
    starting_moves = []
    for _ in range(num_matches):
        # sample two moves without replacement
        one, two = random.choice(len(legal_moves), size=2, replace=False)
        starting_moves.append((legal_moves[one], legal_moves[two]))
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        counts = play_round(agent, test_agents, wins, starting_moves)
